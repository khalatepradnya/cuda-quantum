...
//
// MLIR-mode-only (gated on `!CUDAQ_LIBRARY_MODE` because they consume
// `cudaq::measure_handle`, which is itself MLIR-mode-only). The AST bridge
// intercepts every call inside `__qpu__` regions and lowers it to the
// corresponding `qec.*` MLIR op, so the inline bodies are never reached at
// runtime. They trap with `std::abort()` to mirror the `mz`/`mx`/`my`
// discipline: host code that bypasses the kernel boundary fails loudly
