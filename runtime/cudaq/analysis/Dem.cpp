...
scope make_scope(std::string plugin_name) {
  // `from_plugin` resolves the simulator via dlsym, then forwards to the
  // base `scope` constructor which claims the thread-local slot. The
  // `on_enter` hook drops any residue from a prior run on this thread's
  // copy of the plugin simulator; there is no `on_exit` because the
  // simulator is shared across CUDA-Q infrastructure and clearing state on
  // exit could break a follow-up `cudaq::sample` call.
  std::string name = "dem";
  return scope::from_plugin(
