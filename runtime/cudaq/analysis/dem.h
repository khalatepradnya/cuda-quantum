...
///
/// The returned `scope` claims the thread-local analysis slot, resets the
/// backend's recorded circuit on entry, and releases the slot on destruction.
/// While the scope is alive, every gate, measurement, noise channel, and
/// `qec.*` annotation lowered out of the kernel is appended to the simulator's
/// recorded circuit. `compute_dem` consumes the recorded circuit after the
/// kernel returns and feeds it to Stim's `ErrorAnalyzer`.
///
...
scope make_scope(std::string plugin_name = "stim");
...
runComputeDem(const std::string &kernelName,
